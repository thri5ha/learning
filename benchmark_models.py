        ...
        for batch_size in MAX_NUM_SEQS:
            try:
                llm = LLM(model=model, tokenizer=model,
                          ...
                          **LLM_ARGS)
                print(f"Loaded model  {model} with batch size {batch_size}")
            except Exception as e:
                print(f"Cannot load model, because of {e}")
                continue
            
            prompts = create_filtered_dataset(dataset, tokenzier=model)
...
if __name__ == '__main__':
    ...
    output_folder = '/home/mcw/common/learning/results'
